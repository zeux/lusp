...
#define MODRMSIB(mod, spare, reg) EMIT8((mod << 6) | (spare << 3) | reg), (reg == ESP ? EMIT8(0x24) : (uint8_t*)0)
...
#define MOV_REG_IMM(reg, value) EMIT8(0xb8 + reg), EMIT32(value)
...
#define MOV_PREG_OFF_REG(reg1, offset, reg2) EMIT8(0x89), PREG_OFF(reg1, offset, reg2)
#define MOV_REG_PREG_OFF(reg1, reg2, offset) EMIT8(0x8b), PREG_OFF(reg2, offset, reg1)

#define LEA_REG_PREG_OFF(reg1, reg2, offset) EMIT8(0x8d), PREG_OFF(reg2, offset, reg1)
...
#define JE_IMM8(label) EMIT8(0x74), label = CODE(), EMIT8(0)
